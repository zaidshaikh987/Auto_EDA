...
def display_scatter_plot_of_two_numeric_features(df, num_columns):
    ...
    if len(num_columns) == 1:
        st.info("At least 2 numerical columns are required for a scatter plot")
        return
    
    # Make sure we don't use an index that's out of bounds
    col1, col2 = st.columns(2)
    with col1:
        x_feature = st.selectbox(label="Select X-Axis Feature", options=num_columns, index=0)
    with col2:
        # Set index to 1 if there are at least 2 columns, otherwise 0
        y_index = 1 if len(num_columns) > 1 else 0
        y_feature = st.selectbox(label="Select Y-Axis Feature", 
                               options=[col for col in num_columns if col != x_feature], 
                               index=min(1, len(num_columns)-2))

    if x_feature != y_feature:  # Only plot if different features are selected
        scatter_fig = px.scatter(df, x=x_feature, y=y_feature, 
                               title=f'Scatter Plot: {x_feature} vs {y_feature}')
        st.plotly_chart(scatter_fig, use_container_width=True)
    else:
        st.warning("Please select two different features for the scatter plot")
